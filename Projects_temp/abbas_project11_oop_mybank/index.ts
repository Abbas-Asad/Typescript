...
// Define the Transaction type to keep track of deposits and withdrawals
type Transaction = {
...
class Account {
    ...
    performTransaction(type: 'deposit' | 'withdrawal', amount: number): string {
...
class Bank {
    ...
    async login(): Promise<Account | undefined> {
        console.log("\n=== Log In ===");
        const answers = await inquirer.prompt([
            { type: 'input', name: 'username', message: 'Enter username:' },
            { type: 'password', name: 'password', message: 'Enter password:' }
    ...
    async handleAccountSession(account: Account): Promise<void> {
        while (true) {
